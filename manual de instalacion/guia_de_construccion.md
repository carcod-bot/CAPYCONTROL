...
## 🖥️ Módulo Control POS (2026-07-08 / 2026-08-11)
...
CashRegister (cajas físicas)
  ├── number       → NÂº de caja (único, ej: 003)
...
## ⚙️ Módulo Configuraciones (Usuarios y Roles) - 2026-08-11
...
## 🔒 Seguridad RBAC y Experiencia de Usuario - 2026-08-11
...
- **Rendimiento del Entorno y Cargas de Sesión:**
  - Se configuró el entorno local (.env) para gestionar las sesiones y el caché mediante el sistema de archivos (SESSION_DRIVER=file y CACHE_STORE=file). Esto reduce drásticamente la latencia generada por las continuas consultas a la base de datos que se realizaban en el entorno local.
  - Se ajustó el archivo de inicialización de Node.js actualizándolo a v24 LTS para asegurar que el motor de **Vite 8** y **Tailwind 4** puedan compilar eficientemente los assets en el servidor.
- **Transición y Experiencia de Usuario (UI/UX):**
  - **Dark Mode Dinámico:** Se modificó la funcionalidad de cambio a modo oscuro (	oggleDarkMode en pp.blade.php). En vez de hacer un location.reload() tras la petición, ahora alterna las clases (.dark-mode) directamente en el cliente mediante Javascript.
  - **Transición Global:** Se incorporó en esources/css/app.css una regla base global (	ransition-colors duration-300) que provee animaciones suaves a todos los cambios de fondo, texto y bordes.
- **Correcciones en Módulo de Productos y Búsquedas:**
  - Se solucionó un bug en la vista de inventario de productos (index.blade.php) donde el botón de "Nuevo Producto" apuntaba al ID incorrecto productModal. Se ajustó para apuntar al nombre de la ventana correcto createProductModal.
...
#### Actualizaciones del 11/08/2026:
- **Módulo de Productos Devueltos (Returned Products):**
  - **Trazabilidad Visual:** Se implementó una vista dedicada (`inventory/returned-products/index.blade.php`) para monitorear todos los productos que fueron devueltos en las cajas registradoras (CapyPOS).
  - **Resolución de Estados:** El administrador puede revisar los ítems en estado "Pendiente" y decidir si "Reingresarlos al Stock" o "Desecharlos".
  - **Reingreso Atómico:** Al aprobar un reingreso, el sistema automáticamente regenera un Lote (ProductBatch) con el prefijo `DEV-` y emite un movimiento de ajuste (`InventoryAdjustment`) tipo `in` vinculado estrictamente a la factura original.
- **Estandarización y Corrección de Modales (Stacking Contexts):**
  - **El Bug:** Se solucionó un problema estructural global donde los modales de la aplicación quedaban "atrapados" o recortados dentro de `.main-content` debido a que las animaciones CSS de entrada (e.g. `pageFadeIn` y `opacity`) obligaban al navegador a crear un *Stacking Context*, impidiendo que `position: fixed` se alineara con el Viewport de la pantalla.
  - **El Nuevo Estándar de Construcción:** Para solventar esto y mantener las animaciones limpias, se ha configurado un `@stack('modals')` global en la raíz de `app.blade.php` (justo antes de `</body>`).
  - **Regla Obligatoria:** A partir de ahora, **todo el código HTML de cualquier Modal** en las vistas (como `adjustmentModal`, etc.) **debe estar forzosamente encapsulado** dentro de la directiva `@push('modals') ... @endpush`. Esto asegura que Laravel extraiga el modal del DOM intermedio y lo inserte en la raíz de la página, garantizando una superposición (overlay) perfecta sin heredar opacidades ni transformaciones de sus contenedores padre.
#### Actualizaciones del 15/09/2026:
- **Sistema de Licencias Criptogr�fico:**
  - Implementaci�n de un modelo de licenciamiento anclado al hardware (MAC Address) y al nombre de la empresa, inspirado en Capynom.
  - Creaci�n de la tabla `system_license` y su respectivo modelo Eloquent.
  - Creaci�n del servicio `SysGuard` que valida firmas HMAC locales (usando `APP_INTEGRITY_HASH`) y comprueba el estatus de expiraci�n de las licencias localmente.
  - Implementaci�n de un Middleware Global `CheckLicense` que bloquea rutas cr�ticas y redirige a la vista `/license` en caso de infracci�n.
  - Creaci�n de un notificador silencioso que env�a la telemetr�a de las activaciones v�a correo usando PHPMailer.
  - Creaci�n de los Keygens paralelos para CapyPOS y CapyControl que emiten JSONs base64 cifrados.
